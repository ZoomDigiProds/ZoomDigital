...
namespace InternalProj.Controllers
{
    ...
    public class ViewWorkController : Controller
    {
        ...
        public IActionResult Index(string studio, DateTime? fromDate, DateTime? toDate, int? workTypeId, int currentPage = 1, bool isSearch = false)
        {
            ...
            var viewModel = new WorkOrderViewModel
            {
                StudioList = _context.CustomerRegs.Where(c => c.BranchId == branchId).Select(c => c.StudioName).Distinct().ToList(),
                WorkTypes = _context.WorkTypes.ToList(),
                ResultsSummary = results,
                CurrentPage = currentPage,
                TotalPages = totalPages,
        ...
        [HttpPost]
        public IActionResult DownloadExcel(string studio, DateTime? fromDate, DateTime? toDate, int? workTypeId, int currentPage = 1, int pageSize = 10)
        {
            var branchIdStr = HttpContext.Session.GetString("BranchId");
            if (string.IsNullOrEmpty(branchIdStr) || !int.TryParse(branchIdStr, out int branchId))
            {
            ...
            bool hasAnyFilter = !string.IsNullOrEmpty(studio) || fromDate.HasValue || toDate.HasValue || (workTypeId.HasValue && workTypeId.Value > 0);
            bool shouldShowResults = hasAnyFilter;
            ...
            // ✅ Apply paging so only visible records are exported
            var results = query
                .OrderBy(w => w.Wdate)
                .Skip((currentPage - 1) * pageSize)
                .Take(pageSize)
            ...
            ws.Cell(3, 4).Value = "Work Type";
            ws.Cell(3, 5).Value = "Date";
            ws.Cell(3, 6).Value = "Size";
            ws.Cell(3, 7).Value = "Amount";
            ws.Cell(3, 8).Value = "Advance";
            ws.Cell(3, 9).Value = "Payments";
            ws.Cell(3, 10).Value = "Balance";
            ws.Range(3, 1, 3, 10).Style.Font.SetBold();


            int row = 4, slno = 1;
            decimal totalBalance = 0, totalPayments = 0;

            foreach (var item in results)
            {
                // Get payments for this WorkOrder
                var payments = _context.Receipts
                    .Where(r => r.WorkOrderId == item.WorkOrderId)
                    .Sum(r => (decimal?)r.CurrentAmount) ?? 0;

                decimal balance = (decimal)item.SubTotal - (decimal)(item.Advance ?? 0) - payments;
                totalBalance += balance;
                ...
                ws.Cell(row, 4).Value = item.WorkType?.TypeName ?? "";
                ws.Cell(row, 5).Value = item.Wdate?.ToLocalTime().ToString("dd-MM-yyyy") ?? "";
                ws.Cell(row, 6).Value = item.AlbumSize?.Size ?? "N/A";
                ws.Cell(row, 7).Value = item.SubTotal;
                ws.Cell(row, 8).Value = item.Advance ?? 0;
                ws.Cell(row, 9).Value = payments;
                ws.Cell(row, 10).Value = balance;
                row++;
            }

            // Totals row
            ws.Cell(row, 8).Value = "Totals:";
            ws.Cell(row, 8).Style.Font.SetBold();
            ws.Cell(row, 9).Value = totalPayments;
            ws.Cell(row, 9).Style.Font.SetBold();
            ws.Cell(row, 10).Value = totalBalance;
            ws.Cell(row, 10).Style.Font.SetBold();
        ...
        [HttpPost]
        public IActionResult DownloadPdf(string studio, DateTime? fromDate, DateTime? toDate, int? workTypeId, int currentPage = 1, int pageSize = 10)
        {
            var branchIdStr = HttpContext.Session.GetString("BranchId");
            if (string.IsNullOrEmpty(branchIdStr) || !int.TryParse(branchIdStr, out int branchId))
            {
            ...
            doc.Add(new iText.Layout.Element.Paragraph("Work Order Report")
                .SetFont(boldFont)
                .SetFontSize(16)
                .SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER)
                .SetMarginBottom(10));
            ...
            string fromStr = fromDate?.ToString("dd-MM-yyyy") ?? "N/A";
            string toStr = toDate?.ToString("dd-MM-yyyy") ?? "N/A";
            doc.Add(new iText.Layout.Element.Paragraph($"Date Range: {fromStr} to {toStr}")
                .SetFont(normalFont)
                .SetFontSize(12)
                .SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER)
                .SetMarginBottom(20));
            ...
            // Table header
            string[] headers = { "Sl No", "Work Order No", "Studio", "Work Type", "Date", "Size", "Amount", "Advance", "Payments", "Balance" };
            var table = new iText.Layout.Element.Table(headers.Length).UseAllAvailableWidth();
            foreach (var h in headers)
            {
                table.AddHeaderCell(
                    new iText.Layout.Element.Cell()
                        .Add(new iText.Layout.Element.Paragraph(h).SetFont(boldFont)).SetKeepTogether(true)
                );
            }
            ...
            foreach (var item in results)
            {
                ...
                table.AddCell(new iText.Layout.Element.Paragraph((slno++).ToString()).SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.WorkOrderNo ?? "").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.Customer?.StudioName ?? "").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.WorkType?.TypeName ?? "").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.Wdate?.ToLocalTime().ToString("dd-MM-yyyy") ?? "").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.AlbumSize?.Size ?? "N/A").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.SubTotal.ToString("N2")).SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(item.Advance?.ToString("N2") ?? "0.00").SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(payments.ToString("N2")).SetFont(normalFont));
                table.AddCell(new iText.Layout.Element.Paragraph(balance.ToString("N2")).SetFont(normalFont));
            }


            // Add total row    
            table.AddCell(new Cell(1, 7).Add(new iText.Layout.Element.Paragraph("Totals:").SetFont(boldFont))
                     .SetTextAlignment(iText.Layout.Properties.TextAlignment.RIGHT));
            table.AddCell(new iText.Layout.Element.Paragraph("").SetFont(boldFont)); // empty under Advance
            table.AddCell(new iText.Layout.Element.Paragraph(totalPayments.ToString("N2")).SetFont(boldFont));
            table.AddCell(new iText.Layout.Element.Paragraph(totalBalance.ToString("N2")).SetFont(boldFont));
            table.AddCell(new iText.Layout.Element.Paragraph(totalBalance.ToString("N2")).SetFont(boldFont));
